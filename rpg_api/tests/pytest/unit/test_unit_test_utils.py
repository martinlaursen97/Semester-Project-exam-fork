...
def test_get_user_header_valid_user_id():
    """
    ...
    with (
        ...
        patch('rpg_api.web.api.postgres.auth.auth_utils.create_access_token',
...
def test_get_user_header_no_user_id():
    """
...
def test_get_user_header_invalid_user_id():
    """
    Test that get_user_header raises a TypeError
    when provided with an invalid user_id.
    """
    with pytest.raises(TypeError):
        get_user_header(user_id="not-a-uuid")


#def test_get_user_header_token_creation_failure():
#    """Test that get_user_header raises an Exception when token creation fails."""
#    with (
